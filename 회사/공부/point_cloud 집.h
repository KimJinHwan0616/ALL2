...
class PointCloud {
  ...
  */
  ...
  inline void CopyPointCloud(const PointCloud<PointT> &rhs,
    ...
    // 현재 포인트 클라우드를 비워서 비어있는 상태로 만듭니다.
    // points_.clear();

    // 현재 포인트 클라우드의 크기를 인덱스의 개수에 맞게 조정합니다.
    // points_.resize(indices.indices.size());

    // 인덱스에 해당하는 점들을 rhs 포인트 클라우드에서 현재 포인트 클라우드로 복사합니다.
    // for (std::size_t i = 0; i < indices.indices.size(); ++i) {
    //   points_[i] = rhs.points_[indices.indices[i]];
    // }
  }
