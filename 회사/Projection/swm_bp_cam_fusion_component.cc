  ...
  for (int i =0 ; i < in_box_message->perception_obstacle_size();i++){  
    ...
    if(cluster_point->size() > 1) {
      ...
      if(sub_label == base::ObjectSubType::CAR) {
        labelz = car_gradient;
      } else if(sub_label == base::ObjectSubType::VAN) {
        labelz = van_gradient;
      } else if(sub_label == base::ObjectSubType::TRUCK) {
        labelz = truck_gradient;
      } else if(sub_label == base::ObjectSubType::BUS) {
        labelz = bus_gradient;
      } else if(sub_label == base::ObjectSubType::CYCLIST) {
        labelz = cyclist_gradient;
      } else if(sub_label == base::ObjectSubType::MOTORCYCLIST) {
        labelz = motorcyclist_gradient;
      } else if(sub_label == base::ObjectSubType::TRICYCLIST) {
        labelz = tricyclist_gradient;
      } else if(sub_label == base::ObjectSubType::PEDESTRIAN) {
        labelz = pedestrian_gradient;
